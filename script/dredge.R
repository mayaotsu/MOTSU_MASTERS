...
taape_full_Gam <-gam(presence~s(depth)+year+s(rugosity, k =6)+island+s(mean_1mo_chla_ESA, k =6)
...
df<-readRDS("/Users/mayaotsu/Documents/GitHub/MOTSU_MASTERS/data/spc_edited_cumulative_JUSTMHI_roi") 
taape_MHI <- df[df$species=="LUKA",]
taape_MHI_Gam <-gam(presence~s(depth)+year+s(rugosity, k =6)+island+s(mean_1mo_chla_ESA, k =6)
                    +s(q05_1yr_sst_CRW, k=6)
                    +s(otp_nearshore_sediment, k=6)
                    +s(otp_all_effluent, k=6)
                    +s(MHI_spear, k=6)+
